...
/*making this one cause the vannilla one doesnt wanna go underwater*/
...
public class LeoFollowOwnerGoal extends Goal {
    ...
    public LeoFollowOwnerGoal(TameableEntity tameable, double speed, float minDist, float maxDist, boolean teleportToLeaves) {
    ...
    private boolean tryToTeleportToLocation (int x, int y, int z) {
        if (Math.abs((double)x - this.owner.getPosX()) < 2.0D && Math.abs((double)z - this.owner.getPosZ()) < 2.0D) {
            return false;
        } else if (! this.world.hasWater(new BlockPos(x,y,z))) {
            return false;
        } else {
